			...
			@foreach ($usuariosSucursales as $key => $usuarioSucursal)
				@if(isset($usuarioSucursal))
					<tr>
						...
						<td>
							{{ $usuarioSucursal->sucursal->nombre  }}
						</td>
						<td>
						...
						<td>							
								@if($usuarioSucursal->estado_id == 1 || $usuarioSucursal->estado_id == 3)
									...
									<a href="#" data-popup="tooltip" title="Deshabilitar" onClick = "fnEliminarUsuarioSucursal({{ $usuarioSucursal->id }});"><i class="fa fa-fw fa-thumbs-o-down"></i></a>
								@endif									

								@if($usuarioSucursal->estado_id == 2)
									<a href="#" data-popup="tooltip" title="Habilitar" onClick = "fnHabilitarUsuarioSucursal({{ $usuarioSucursal->id }});"><i class="fa fa-fw fa-thumbs-o-up"></i></a>
								@endif
						</td>
