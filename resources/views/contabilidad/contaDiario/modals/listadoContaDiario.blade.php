				...
				@foreach ($datosContaDiario as $key => $dato)
					@if ($dato->contrato_id == 0)
						...
						<tr>
							...
							<td>{{ $dato->tcom }}</td>
							<td>{{ $dato->ref }}</td>
							<td><a href="#" onClick = "fnEditarContaDiario({{ $dato }});" data-popup="tooltip" title="Editar" data-toggle="modal" data-target="#modalUpdateContaDiario" data-keyboard="false"><i class="fa fa-fw fa-edit"></i></a>
								<a href="#" data-popup="tooltip" title="Eliminar" onClick = "fnEliminarContaDiario({{ $dato->id }});"><i class="fa fa-fw fa-trash-o"></i></a></td>	
						</tr>
					@else
						@if ($dato->contrato1)
							@php
